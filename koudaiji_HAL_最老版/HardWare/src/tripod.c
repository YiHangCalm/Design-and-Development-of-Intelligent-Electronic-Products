...
uint16_t Y_OFFSET = 545;  //校准完成后激光照射在Y轴上的坐标
...
void Tripod_Init(void)
{
    ...
    Tripod_X_DIR.GPIOx = GPIOD;     Tripod_X_DIR.GPIO_Pin = GPIO_PIN_1;    //D1 -> X_DIR
    Tripod_X_EN.GPIOx = GPIOA;      Tripod_X_EN.GPIO_Pin = GPIO_PIN_15;    //A15 -> X_EN
    Tripod_X_STEP.GPIOx = GPIOG;    Tripod_X_STEP.GPIO_Pin = GPIO_PIN_15;  //G15 -> X_STEP
    Tripod_X_LIMIT.GPIOx = GPIOB;   Tripod_X_LIMIT.GPIO_Pin = GPIO_PIN_6;  //B6 -> X限位信号读取
    Tripod_Y_DIR.GPIOx = GPIOE;     Tripod_Y_DIR.GPIO_Pin = GPIO_PIN_0;    //E0 -> Y_DIR
    Tripod_Y_EN.GPIOx = GPIOE;      Tripod_Y_EN.GPIO_Pin = GPIO_PIN_7;     //E7 -> Y_EN
    Tripod_Y_STEP.GPIOx = GPIOE;    Tripod_Y_STEP.GPIO_Pin = GPIO_PIN_9;   //E9 -> Y_STEP
    Tripod_Y_LIMIT.GPIOx = GPIOE;   Tripod_Y_LIMIT.GPIO_Pin = GPIO_PIN_11; //E11 -> Y限位信号读取
...
void Tripod_Y_Coord(int y_dat)
{
	...
	temp = Y_OFFSET - temp;//
	temp = 0 - temp;
	x_len = hypot((Tripod_X.coord - 665), DIS_X);  //旋转角度矫正
