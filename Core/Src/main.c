...
int main(void)
{
	...
	if(Touch_Key_Value == TOUCH_KEY_C) //如果检测到按键“C”按下
	{
		...
		if(Touch_Key_Value == TOUCH_KEY_C) //延时两秒之后还能检测到按键“C”按下
		{
			...
			adc_value= new uint32_t[1024];
			//uint32_t* adc_value = new uint32_t[1024]; // 申请内存，用于存放ADC采集的数据
	...
	if(Touch_Key_Value == TOUCH_KEY_D)
	{
		...
		if(Touch_Key_Value == TOUCH_KEY_D)
		{
			...
			DiWenLcd_Printf("<:切换一次检测的形状和颜色");
            DiWenLcd_Printf("1秒一次打印返回的坐标值");
			DiWenLcd_Printf(">:停止测试AI通信，停止打印坐标");
...
//	HAL_Delay(3000);  //等待功率输出稳定
    ...
    Lcd_printf("----- Program Version V2.0.1 -----");
